<?php
use yii\grid\GridView;
//use yii\widgets\Breadcrumbs;
use yii\helpers\Url;
use yii\widgets\Menu;
use yii\helpers\Html;
use common\models\Tourcate;
use yii\helpers\ArrayHelper;
?>
<h1 class="title-page"><?php echo Yii::t('appbk','Category Tour Manager');?></h1>
<?php
echo Menu::widget(array(
    'items'=>array(  
                array('label' => 'Add new', 'url' => array('tourcate/create')),
               //array('label' => 'Login', 'url' =>array('site/login'), 'visible' => Yii::$app->user->isGuest),
    ),
    'options'=>array('class'=>'nav nav-tabs'),
));
echo GridView::widget(array(
    'dataProvider' => $dataProvider,
    'filterModel' => $model,
    'id'=>'gridHotelID',// ID to grid wrapper
    'columns' => array(    
           //array('class' => 'yii\grid\SerialColumn'),
            array(
                'attribute' => 'id',     
                'headerOptions' =>array('style'=>'width:5%;'),           
            ),
            array(
               'attribute' => 'title',
               'label'=>Yii::t('app','Title'),
               'format' => 'raw',
               'value'=>function ($data) {
                     return Html::a(Html::encode($data->title),array('id'=>$data->id,'update'));
                },
            ),
            array(
                'label'=>'Parent',
                'attribute' => 'parent_id',
                'headerOptions' =>array('style'=>'width:20%;'),
                'filter'=>$allcate,
                'value' =>function($data) {
                    return $data->getNameTourCate($data->parent_id);
                },
            ),
        array(
                'label'=>Yii::t('app','Country'),   
                'attribute' =>'country_id',  
                'format' => 'raw',                
                'filter'=>ArrayHelper::map($countrys,'id', 'name'),
                'headerOptions' =>array('style'=>'width:10%;'),  
                'value'=>function ($data) {
                     return $data->country->name;
                },         
            ),
            array(
                'attribute' => 'path',     
                'headerOptions' =>array('style'=>'width:10%;'),           
            ),  
             array(
                'label'=>Yii::t('app','Status'),
                'attribute' => 'status', 
                'format'   => 'raw',
                'headerOptions' =>array('style'=>'width:6%;'),  
                'filter'=>Tourcate::getAllStatus(),        
                'value'    =>function($data) {
                    return Tourcate::getStatus($data->status);
                }, 
               // 'filter'=>array("0"=>"No","1"=>"Yes"),            
            ),
                     
             array(
                  'class' => 'yii\grid\ActionColumn',
                  'headerOptions' =>array('style'=>'width:5%;text-align: center;'),  
                  'template' => '{delete}',
             ),
        ),
         'tableOptions' =>array('class' => 'uk-table uk-table-hover uk-table-striped uk-table-condensed'),
         'pager' =>array('options' =>array('class' => 'uk-pagination')),
));
?>